...
description: "LofiTick.Desktop.UI Nuxt 5/Vue/TypeScript 前端编码规范。适用于页面、组件、composable、store、server API、i18n、Tauri 集成、配置与测试。"
---

> **Skill 文档维护优先级（强制）**
>
> - 本 skill 由原 `.github/copilot-instructions.md` 完整迁移而来，原有规范不得丢失。
> - 本项目不再把根目录 `docs/` 作为 AI 规则文档来源；AI 编码规则、能力边界、复用说明与稳定约束统一维护在 `.github/skills/**/SKILL.md`。
> - 每次新增、修改、删除页面、组件、composable、store、server API、i18n、配置、测试或能力边界时，必须同步检查相关 skill 文档是否需要更新；若实现与本文档不一致，必须修正文档或实现，不能让规则过期。
> - `.github/skills/SKILL.md` 只负责索引子 skill；具体规范必须维护在子目录的 `SKILL.md` 中，便于 GitHub Copilot 按需加载。
...
- 强制：本项目的 AI 规则与能力说明统一维护在 `.github/skills/**/SKILL.md`，禁止继续把根目录 `docs/` 作为 AI 规则文档来源。
- 强制：`.github/skills/SKILL.md` 是唯一子 skill 索引入口；所有文件索引链接只能写在该根 `SKILL.md`，子 skill 禁止再维护跨目录索引。
- 强制：子 skill 必须按职责拆分；同一类规则只能有一个权威来源，禁止在多个 skill 中复制粘贴后并行维护。
- 强制：只有真实存在、并且确实承担规则职责的主题才允许创建子 skill 目录；禁止为了凑层级人为新增空目录或空 skill。
- 强制：页面、组件、composable、store、共享类型、server API、i18n、Tauri 集成规则都必须在对应 skill 中写清职责、依赖项、被谁依赖、以及和同级/父级模块的关系。
- 强制：新增、移动、删除源码或子 skill 时，必须同步修正对应 skill 内容与 `.github/skills/SKILL.md` 的下一层链接。
...
- 强制：只要 `@vueuse/nuxt` 或 `es-toolkit` 已有现成能力，就必须优先使用它们或仓库里已有的对应封装；必须按当前场景选择更合适的那个，禁止在本地再手写一套同类实现去平行替代。
...
- 强制：当发现当前功能存在更合适的第三方依赖包时，可以主动给出 1~3 个候选并说明取舍；推荐的候选必须是仍然活跃、社区使用广、维护及时且综合效率更高的包，不能为了“有包可用”随便推荐冷门或停更依赖。
...
- 强制：组件、页面片段、composable、工具函数都要尽量原子化，单个函数优先控制在 30~50 行以内，能压到 30 行左右就不要放大到临界边缘。
- 强制：单个 `.vue` 文件优先控制在 300~500 行以内；若因页面复杂度必须超过上限，必须先拆出 section、子组件或共享 composable，再保留最终必要的页面壳。
- 强制：前端可复用的“轮子”必须在对应 skill 文档里列清楚，包括 `app/components/**`、`app/composables/**`、`app/stores/**`、`shared/**`、现有 API composable 与 Tauri 封装；skill 必须说明这些轮子分别负责什么、谁在复用、为什么不重复造。
...
- 组件内的延迟显示、延迟隐藏、定时重置、一次性超时与轮询刷新，默认也必须优先复用 `VueUse` 的 `useTimeoutFn` / `useIntervalFn` / `useDebounceFn` / `useThrottleFn`，禁止继续新增裸 `setTimeout`、`clearTimeout`、`setInterval`、`clearInterval`。
- 只有在明确属于脚本工具、编译产物处理或无法引入 Vue 运行时语义的场景，才允许用 Node / `es-toolkit` 等等价实现；若必须保留底层计时器，也要说明原因并把清理语义收口成单一封装。
...
- 断点来自 [`app/assets/css/main.css`](../../../app/assets/css/main.css) 的 `@theme` 定义，除 Tailwind 默认断点外，项目额外提供：
  ...
  - 按钮图标颜色默认应保持与按钮内容协调的浅色层级；当前项目通常使用 `ui.leadingIcon: 'text-muted'` 或等价 slot class，但当按钮本身是高对比实色语义按钮时，图标应优先采用白色或与按钮语义匹配的可读性颜色，禁止一刀切把所有按钮图标都固定成 `text-muted`，也禁止让同语义按钮在不同位置出现明显不同的图标层级。
  - 若按钮图标或输入框 leading/trailing 图标在默认主题下已经是最合适、最清楚的颜色，就不要再人为干预；尤其是 `UInput`、`UInputTime`、`USelect` 这类输入控件的前后装饰图标，默认优先保持组件自身的主题输出，只有在明确需要强调语义或对比度时才允许覆盖颜色。
  - 图标一致性的验收标准必须看实际渲染效果，不能只看 token、类名、或图标名称一致；如果两枚按钮在代码上同样的图标和同样的颜色类，实际呈现仍有差异，则需要按渲染结果继续调到视觉一致。
  - Chrome 校准引导里的“复制扩展目录”按钮必须放在弹窗标题区或同级操作区，不得在正文里重复展示完整扩展目录路径；正文只保留步骤与风险提示。
  - 删除、重置等危险操作按钮默认状态不得直接使用危险色；仅允许在 hover / focus / active 等交互态显式变为危险色，二次确认内的最终确认按钮除外。

  ### Skill 同步（强制）
  - 强制：本规则的权威来源为 `.github/skills/**/SKILL.md`，`.github/copilot-instructions.md` 只保留加载入口。
  - 强制：当你对任一处规则段落进行新增、修改或删除时，必须同步检查同项目其他 skill 是否存在重复或过期规则，并保证 `.github/skills/SKILL.md` 的子 skill 索引准确。
...
- TS/JS 函数声明默认优先使用箭头函数；只有在确实需要函数提升、构造函数、原型方法、装饰器/框架约定或必须保留动态 `this` 绑定时，才允许使用 `function` 声明或表达式。
...
- 除简体中文外，`zh.tw.ts` / `en.ts` / `ja.ts` 必须与 `zh.cn.ts` 一一对应，不能多 key，也不能少 key。
- 翻译必须准确、自然、精炼，禁止生硬直译；其中英文必须优先做到精准、简洁、可直接使用。
