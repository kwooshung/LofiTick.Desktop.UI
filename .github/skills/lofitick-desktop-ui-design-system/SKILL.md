---
name: lofitick-desktop-ui-design-system
description: 'Token architecture, component specifications, and slide generation. Three-layer tokens (primitive→semantic→component), CSS variables, spacing/typography scales, component specs, strategic slide creation. Use for design tokens, systematic design, brand-compliant presentations. (Token 架构、组件规格与幻灯片生成；包含三层 token（primitive→semantic→component）、CSS 变量、间距与排版尺度、组件规格和策略型幻灯片创建；用于设计 token、系统化设计和符合品牌规范的演示文稿。)'
