...
- 单个 `*.vue` 文件的常规警戒线是不超过 300 行；如果生成的组件预计超过 300 行，必须主动提醒用户并建议拆分方案。
- 单个 `*.vue` 文件原则上不超过 400 行；超过 400 行必须拆分。
- 复杂页面的 Page 组件可以适当放宽到 500 行，但超过 500 行必须拆成子组件或 composables。
- 组件和页面都必须优先保持职责单一；当布局、状态、数据转换和复杂交互混在一起时，必须拆分。
