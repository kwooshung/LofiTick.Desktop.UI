---
name: 'lofi-nuxt-ui'
description: 'LofiTick Nuxt UI 规范助手。当用户询问 Nuxt/Vue/TypeScript 前端开发、页面组件开发、API 调用或 Tauri 集成相关问题时自动调用。强制遵守 .trae/shared/rules.md 中的所有规则。'
---
...
- [1. 项目与工作区](SKILL.md#1-项目与工作区)
- [2. 执行流程（改动前门禁）](SKILL.md#2-执行流程改动前门禁)
- [2.1 规则同步（强制）](SKILL.md#21-规则同步强制)
- [3. 工程与依赖约束](SKILL.md#3-工程与依赖约束)
- [3.2.1 导入路径（强制）](SKILL.md#321-导入路径强制)
- [4. API 请求规范](SKILL.md#4-api-请求规范)
- [5. TypeScript 与注释规范](SKILL.md#5-typescript-与注释规范)
- [6. Pages 特例（零容忍）](SKILL.md#6-pages-特例零容忍)
- [7. i18n 规则](SKILL.md#7-i18n-规则)
- [8. Tauri 集成规范](SKILL.md#8-tauri-集成规范)
- [9. Skills 与冲突处理](SKILL.md#9-skills-与冲突处理)
- [10. 参考与示例](SKILL.md#10-参考与示例)
...
- 强制：本项目的文档结构必须与源码树一一对应；`app/**`、`shared/**`、`server/**`、`i18n/**` 里每个真实存在的源码文件都要有对应 Markdown 文档，目录总览使用 `index.md`，类型页使用 `index.types.md`。
- 强制：根目录只保留 `README.md`；其余目录只保留 `index.md` 作为目录入口，禁止在子目录里再放 `README.md`。
- 强制：`index.md` 只能链接下一层的子文档，不能跨多层直链；如果某个目录下存在 `index.types.md`，`index.md` 必须显式链接它。
- 强制：只有源码里真实存在、并且确实承担职责的目录才允许创建对应 `index.md`；禁止为了凑层级人为新增空目录或空索引。
- 强制：页面、组件、composable、store、共享类型、服务端 API、i18n 规则页都必须有自己的文档，文档必须写清楚职责、依赖项、被谁依赖、以及和同级/父级模块的关系。
- 强制：新增、移动、删除源码时，必须同步新增、移动、删除对应文档，并同步更新父级 `index.md` 的下一层链接。
...
- 只有在明确属于脚本工具、编译产物处理或无法引入 Vue 运行时语义的场景，才允许用 Node / `es-toolkit` 等价实现；若必须保留底层计时器，也要说明原因并把清理语义收口成单一封装。
