<template>
  <CrawlersNodesCommonBasic icon-name="i-lucide-plus" :title="t('components.crawler.blueprint.nodes.math.add.title')" :description="t('components.crawler.blueprint.nodes.math.add.description')" header-bg="bg-lime-500" :left-pins="computedLeftPins" :right-pins="rightPins">
    <div class="space-y-3">
...
const computedLeftPins = computed<IBasicSidePin[]>(() => {
  return stateOperandIds.value.map((operandId, index) => ({
    ...
    description: t('components.crawler.blueprint.nodes.math.add.pinDescriptions.operand', { label: labelFromIndex(index).toUpperCase() })
  }));
});
...
const rightPins: IBasicSidePin[] = [
  {
    ...
    dataType: 'number',
    topPercent: 35,
    description: t('components.crawler.blueprint.nodes.math.add.pinDescriptions.result')
  },
