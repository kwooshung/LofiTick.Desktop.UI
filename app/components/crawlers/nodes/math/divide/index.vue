  ...
  <CrawlersNodesCommonBasic icon-name="i-lucide-divide" :title="t('components.crawler.blueprint.nodes.math.divide.title')" :description="t('components.crawler.blueprint.nodes.math.divide.description')" header-color="" header-bg="bg-lime-500" :left-pins="computedLeftPins" :right-pins="rightPins">
    <div class="space-y-3">
      <UFormField v-for="(operandId, index) in stateOperandIds" :key="operandId" :label="t('components.crawler.blueprint.nodes.common.operandInputLabel', { label: labelFromIndex(index).toUpperCase() })">
        ...
        <UInputNumber
          v-else
          :id="`crawlerMathDivide-${operandId}`"
...
import type { IMathDivideNodeData } from '@/components/crawlers/nodes/math/divide/index.types';
...
watchEffect(() => {
   ...
   * 常量：rawOperandIds。
   */
  const rawOperandIds = (stateNode.node.data as IMathDivideNodeData | undefined)?.operandIds;
  /**
   * 常量：rawOperandValues。
   */
  const rawOperandValues = (stateNode.node.data as IMathDivideNodeData | undefined)?.operandValues;
