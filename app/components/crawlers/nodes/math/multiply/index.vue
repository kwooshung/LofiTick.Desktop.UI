...
interface IMathMultiplyNodeData {
  operandIds?: string[];
...
const computedLeftPins = computed<IBasicSidePin[]>(() => {
  return stateOperandIds.value.map((operandId, index) => ({
    ...
    description: `乘数 ${labelFromIndex(index).toUpperCase()}`
  }));
});
...
const rightPins: IBasicSidePin[] = [
  { id: 'result-number', label: 'result', direction: 'out', dataType: 'number', topPercent: 35, description: '乘法结果' },
