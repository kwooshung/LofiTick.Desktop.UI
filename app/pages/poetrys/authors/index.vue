  ...
  <DashboardPage>
    <div class="flex w-full flex-1 gap-1">
      <div class="flex-1">
        <UTable
          :columns="columns"
          :data="computedPoetryAuthorsData"
          :loading="loading"
          class="shrink-0"
          sticky
          :ui="{
...
import type { IQueryResultPoetryAuthorsSummaryPage } from '@@/server/db/mysql/quick/poetrys/authors/index.types';
 ...
 * API：诗词搜索
 */
const { datas, loading, refreshDebounced } = await useApi<IQueryResultPoetryAuthorsSummaryPage>('poetrys/authors', { datas: buildApiQueryFromRoute(), immediate: true });

/**
 * 计算属性：直接映射成表格需要的格式
 */
const computedPoetryAuthorsData = computed<IPageTableColumnPoetryAuthors[]>(() => {
 ...
 * 常量：表格列定义
 */
const columns: TableColumn<IPageTableColumnPoetryAuthors>[] = [
  {
    accessorKey: 'id',
    meta: {
      class: {
