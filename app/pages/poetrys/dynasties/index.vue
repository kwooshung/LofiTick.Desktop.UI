  ...
  <DashboardPage>
    <div class="flex w-full flex-1 gap-1">
      <div class="flex-1">
        <UTable
          :columns="columns"
          :data="computedProetryDynastiesDatas"
          :loading="loading"
          class="shrink-0"
          sticky
          :ui="{
...
const toggleSort = (field: 'id' | 'count' | 'updated' | 'created') => {
  ...
  q.order_by = nextBy;
  q.order_dir = nextDir;
  navigateTo({ path: '/poetrys/dynasties', query: q });
};

/**
 * API：诗词搜索
 */
const { datas, loading, refreshDebounced } = await useApi<IQueryResultPoetryDynastiesSummaryPage>('poetrys/dynasties', { datas: buildApiQueryFromRoute(), immediate: true });

/**
 * 计算属性：直接映射成表格需要的格式
 */
const computedProetryDynastiesDatas = computed<IPageTableColumnPoetryDynasties[]>(() => {
 ...
 * 常量：表格列定义
 */
const columns: TableColumn<IPageTableColumnPoetryDynasties>[] = [
  {
    accessorKey: 'id',
    meta: {
      class: {
