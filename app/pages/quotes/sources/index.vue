...
const navigateWithSingleFilter = (sourceId: number | string) => {
 ...
 * API：名句出处
 */
const { datas, loading, refreshDebounced } = await useApi<IQueryResultQuoteSourcesSummaryPage>('quotes/sources', { datas: buildApiQueryFromRoute(), immediate: true });

/**
 * 计算属性：名句出处表格数据
 */
const computedQuoteSourcesData = computed<IPageTableColumnQuoteSources[]>(() => {
 ...
 * 常量：表格列
 */
const columns: TableColumn<IPageTableColumnQuoteSources>[] = [
  {
    accessorKey: 'id',
    meta: {
      class: {
