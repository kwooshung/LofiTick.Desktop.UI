...
const HOTSEARCH_PODCAST_AUDIO_URLS = ['https://cdn.pixabay.com/audio/2026/01/25/audio_1dd7f3126d.mp3', 'https://cdn.pixabay.com/audio/2026/02/20/audio_2b2627eddc.mp3'];
...
export const hotsearchDataRowsGet = (date: string): IHotsearchDataRow[] => {
  ...
  return Array.from({ length: 28 }, (_, index) => {
    ...
    const rank = index + 1;
    const topic = HOTSEARCH_SAMPLE_TOPICS[index % HOTSEARCH_SAMPLE_TOPICS.length];
...
export const hotsearchPodcastViewGet = (date: string, variant: THotsearchPodcastVariantKey, mediaPlatformKey?: string): IHotsearchPodcastViewModel => {
  ...
  const sentences: IHotsearchPodcastSentence[] = Array.from({ length: variant.endsWith('short') ? 5 : 8 }, (_, index) => ({
    ...
    durationLabel: `${12 + index}s`,
    audioUrl: HOTSEARCH_PODCAST_AUDIO_URLS[index % HOTSEARCH_PODCAST_AUDIO_URLS.length] ?? HOTSEARCH_PODCAST_AUDIO_URLS[0],
    waveformPath: index % 2 === 0 ? HOTSEARCH_SAMPLE_WAVEFORM_PRIMARY : HOTSEARCH_SAMPLE_WAVEFORM_SECONDARY
  }));
  const assets: IHotsearchPodcastMediaAsset[] = [
    {
      ...
      sources: [
        {
          src: HOTSEARCH_PODCAST_AUDIO_URLS[0],
          type: 'audio/mpeg',
          size: 10
        }
      ]
